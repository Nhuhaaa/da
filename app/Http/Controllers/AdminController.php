...
class AdminController extends Controller
{
    ...
    {{
        ...
        $totalRevenue = Order::sum('total'); // Doanh thu tổng

        // Lấy các dịch vụ mới nhất (ví dụ: 5 dịch vụ gần nhất)
        $latestServices = Service::orderBy('created_at', 'desc')->take(5)->get();
        return view('admin.admin', compact('totalProducts', 'totalBookings', 'totalServices', 'totalPackages', 'totalUsers', 'totalOrders', 'totalRevenue', 'totalBlogs', 'latestServices'));
    }}
...
// //     \/       ============================================================
    ...
    {
        $categoryService = Category_service::find($id);

        if ($categoryService) {
            // Kiểm tra nếu danh mục này có dịch vụ liên quan
            if ($categoryService->services()->exists()) {
                return redirect()->route('category_sv_admin')->with('error', 'Không thể xóa danh mục vì có dịch vụ liên quan.');
            }

            // Xóa danh mục nếu không có dịch vụ liên quan
...
// //     \/       ============================================================
    ...
    public function index()
    {
        ...
        $allService = Service::with('category')->orderBy('id', 'desc')->paginate(4); // Lấy dịch vụ và thông tin danh mục
        return view('admin.service_admin', compact('allService', 'categories'));
    }
    ...
    public function addService(Request $request)
    {
        ...
        $request->validate([
            ...
            'category_id_sv' => 'required|exists:category_service,id', // Kiểm tra category_id phải tồn tại
        ]);

        // Lưu dịch vụ vào cơ sở dữ liệu
        $data = $request->only(['service_name', 'description', 'price', 'category_id_sv']);
    ...
    public function deleteService($id)
    {
        ...
        if ($service) {
            ...
            return redirect()->route('service_admin')->with('success', 'Sản phẩm đã được xóa thành công.');
        } else {
            return redirect()->route('service_admin')->with('error', 'Không tìm thấy sản phẩm.');
        }
    }
    ...
    public function editService($id)
    {
        ...
        if ($service) {
            return view('admin.update_service', compact('service', 'categories'));
        } else {
    ...
    public function updateService(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'price' => 'required|numeric',
            'image' => 'image|mimes:jpeg,png,jpg,gif,webp,svg|max:2048',
            'category_id_sv' => 'required|exists:category_service,id',
        ]);

        $service->service_name = $request->input('service_name');
        $service->description = $request->input('description');
        $service->price = $request->input('price');
        $service->category_id_sv = $request->input('category_id_sv');
...
// //     \/       ============================================================
    ...
    public function add_blog(Request $request)
    {
        $request->validate([
            ...
            'image' => 'image|mimes:jpeg,png,jpg,gif,webp,svg|max:2048', // Kiểm tra hình ảnh nếu có
        ]);
